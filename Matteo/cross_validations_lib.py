...
def cross_validation_SVM(Y,X, degrees, C_parameters, kernel_types,k_fold, seed, max_iters):
    
    # Get the indices so that we get the k'th subgroup in test, others in train, for each k
    k_indices = build_k_indices(Y, k_fold, seed)
    
    # Initialize matrix of computed accuracies for each degree and each fold
    accuracies_train_by_fold = np.zeros([len(degrees), len(C_parameters),len(kernel_types), k_fold])
    accuracies_test_by_fold = np.zeros([len(degrees), len(C_parameters),len(kernel_types), k_fold])
    ...
    for k in range(k_fold):
        ...
        accuracies_train_by_fold[:,:,:,k], accuracies_test_by_fold[:,:,:,k] = cross_validation_one_fold_SVM\
            (Y_cross_val_train, Y_cross_val_test, X_cross_val_train, X_cross_val_test, \
                                 degrees, C_parameters, kernel_types,max_iters)
    # Compute the mean accuracies over the folds, for each degree
    ...
    # Get the index of the best accuracy in the testing set
    max_id_deg_test,max_id_C_parameter, max_id_kernel= \
        np.unravel_index(mean_accuracies_test_by_deg.argmax(), mean_accuracies_test_by_deg.shape)
    ...
    best_accuracy_test = mean_accuracies_test_by_deg[max_id_deg_test,max_id_C_parameter,max_id_kernel]
    corresponding_accuracy_train = mean_accuracies_train_by_deg[max_id_deg_test,max_id_C_parameter,max_id_kernel]
    
    print('Best accuracy test =', best_accuracy_test, 'with degree =', best_deg , 'Penalty parameter=',best_C_parameter, 'kernel type=',best_kernel_type)
    print('Corresponding accuracy train =', corresponding_accuracy_train)
    
    return best_deg, best_C_parameter, best_kernel_type, best_accuracy_test, corresponding_accuracy_train
...
def cross_validation_one_fold_SVM(y_cross_val_train, y_cross_val_test, tx_cross_val_train, tx_cross_val_test, \
    ...
    accuracies_train_by_deg = np.zeros([len(degrees),len(C_parameters),len(kernel_types)])
    accuracies_test_by_deg = np.zeros([len(degrees),len(C_parameters),len(kernel_types)])
    ...
    for deg_id, deg in enumerate(degrees):
        ...
        tol=0.001, verbose=False)
                ...
                # Compute the accuracies for each degree
                    accuracies_train_by_deg[deg_id,C_id,kernel_id] = \
                        np.sum(y_predicted_cross_val_train == y_cross_val_train)/len(y_cross_val_train)
                    accuracies_test_by_deg[deg_id,C_id,kernel_id] = \
                        np.sum(y_predicted_cross_val_test == y_cross_val_test)/len(y_cross_val_test)


                    print(accuracies_test_by_deg[deg_id,C_id,kernel_id],accuracies_train_by_deg[deg_id,C_id,kernel_id] )
